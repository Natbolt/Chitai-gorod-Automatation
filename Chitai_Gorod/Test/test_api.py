...
def test_1():
    ...
    with allure.step("Проверить, что id товара в корзине совпадает с id выбранного для добавления в коразину товара"):
        assert id == id_added

@allure.id ("SKYPRO-2")
@allure.epic("Корзина Читай-город. АРТ")
@allure.story ("Измунение количества товаров в корзине")
...
def test_4():
    with allure.step("Добавить в корзину все товары из списка"):
        goods = ['Настольная игра Каркассон, Hobby World', 'Настольная игра GaGa Games "Большая Бродилка', 'Акриловая краска орхидея olki, 100 мл']
        total_cost = 0
        for i in goods:
